...
const SatellitesSection = () => {
  ...
  const handlePurchase = async () => {
    ...
    if ((userOwnedCount + numQuantity) > selectedSatellite.purchaseLimit) {
        toast({ variant: "destructive", title: "Límite alcanzado", description: `No puedes comprar esta cantidad. El límite para esta moneda es ${selectedSatellite.purchaseLimit} y ya posees ${userOwnedCount}.` });
        return;
    }
  ...
  return (
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Usa tu saldo para comprar monedas y obtener rendimientos.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {satellites.length > 0 ? satellites.map((satellite) => {
            ...
            return (
              <Card key={satellite.id} className="overflow-hidden flex flex-row items-center gap-4 p-4">
                ...
                <div className="flex-grow">
                    ...
                    <Button className="w-full mt-3 h-9" onClick={() => openConfirmationDialog(satellite)} disabled={!user}>
            ...
            <div className="text-center text-muted-foreground py-8 col-span-full">
                <p>No hay monedas disponibles para la compra en este momento.</p>
                <p className="text-sm">Por favor, pídele a un administrador que añada monedas nuevas.</p>
            </div>
        )}
      </CardContent>
    </Card>
