...
type ReferredUserWithSatellites = User & { purchasedSatellites: PurchasedSatellite[] };

const ReferralSection = () => {
    const { user } = useAuth();
    const [referredUsers, setReferredUsers] = useState<ReferredUserWithSatellites[]>([]);
        ...
        if (user && user.referredUsers && user.referredUsers.length > 0) {
            ...
            const unsubscribe = onSnapshot(q, async (querySnapshot) => {
                const referralsDataPromises = querySnapshot.docs.map(async (doc) => {
                    ...
                    const satellitesColRef = collection(db, `users/${userData.id}/purchasedSatellites`);
                    const satellitesSnapshot = await getDocs(satellitesColRef);
                    const purchasedSatellites = satellitesSnapshot.docs.map(satelliteDoc => {
                         const satelliteData = satelliteDoc.data();
                         return {
                            id: satelliteDoc.id,
                            ...satelliteData,
                            purchaseDate: (satelliteData.purchaseDate as Timestamp).toDate(),
                         } as PurchasedSatellite
                    });

                    return { ...userData, purchasedSatellites };
                });
                                    ...
                                    <AccordionTrigger className="px-4 py-3 hover:no-underline">
                                        <div className="flex justify-between items-center w-full">
                                            ...
                                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                                ...
                                                <span>{ref.purchasedSatellites.length} Compras</span>
                                            </div>
                                        </div>
                                    </AccordionTrigger>
                                    <AccordionContent className="px-4 pb-3">
                                        {ref.purchasedSatellites.length > 0 ? (
                                             <Table>
                                                <TableHeader>
                                                    <TableRow>
                                                ...
                                                <TableBody>
                                                    {ref.purchasedSatellites.map(sat => (
                                                        <TableRow key={sat.id}>
                                                            <TableCell>{sat.name}</TableCell>
                                                            <TableCell>{sat.purchaseDate.toLocaleDateString('es-CO')}</TableCell>
                                                            <TableCell className="text-right">{new Intl.NumberFormat("es-CO", { style: "currency", currency: "COP", maximumFractionDigits: 0 }).format(sat.price)}</TableCell>
                                                        </TableRow>
                                                    ))}
                                                </TableBody>
                                            </Table>
                                        ) : (
                                            <p className="text-sm text-muted-foreground text-center py-4">Este referido aún no ha comprado monedas.</p>
                                        )}
                                    </AccordionContent>
                                </AccordionItem>
                             ))}
