    ...
    <div className="animate-in fade-in-20 slide-in-from-bottom-8 duration-500">
      <form className="space-y-6" onSubmit={handleSubmit(onSignInSubmit)}>
        <div className="space-y-2">
          ...
          <Input {...register("phone", { required: true })} id="phone" type="tel" placeholder="3001234567" required className="bg-muted/50 focus:bg-card" disabled={isSubmitting} />
        </div>
        <div className="space-y-2">
          <Label htmlFor="password">Contraseña</Label>
          <Input {...register("password", { required: true })} id="password" type="password" placeholder="••••••••" required className="bg-muted/50 focus:bg-card" disabled={isSubmitting} />
        </div>
