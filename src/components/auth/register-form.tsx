    ...
    <div className="animate-in fade-in-20 slide-in-from-bottom-8 duration-500">
      <form className="space-y-6" onSubmit={handleSubmit(onRegisterSubmit)}>
        <div className="space-y-2">
          ...
          <Input {...register("phone", { required: "El número es requerido" })} id="phone" type="tel" placeholder="3001234567" required className="bg-muted/50 focus:bg-card" disabled={isSubmitting} />
           {errors.phone && <p className="text-xs text-destructive">{errors.phone.message}</p>}
        </div>
        <div className="space-y-2">
          <Label htmlFor="password">Contraseña</Label>
          <Input {...register("password", { required: "La contraseña es requerida", minLength: { value: 6, message: "La contraseña debe tener al menos 6 caracteres" } })} id="password" type="password" placeholder="••••••••" required className="bg-muted/50 focus:bg-card" disabled={isSubmitting} />
        ...
        <div className="space-y-2">
          ...
          <Input {...register("referralCode")} id="referralCode" type="text" placeholder="ABCXYZ" className="bg-muted/50 focus:bg-card uppercase" disabled={isSubmitting} />
        </div>
