...
const SatelliteManagement = () => {
    ...
    const onAddSubmit = async (values: z.infer<typeof satelliteSchema>) => {
        try {
            ...
            toast({ title: "Moneda Creada", description: `La moneda "${values.name}" ha sido añadida.` });
            addForm.reset();
            setSatelliteImageDataUrl(null);
            ...
            toast({ variant: "destructive", title: "Error", description: "No se pudo crear la moneda." });
        }
    };
    ...
    const onEditSubmit = async (values: z.infer<typeof satelliteSchema>) => {
        ...
        try {
            ...
            toast({ variant: "destructive", title: "Error", description: "No se pudo actualizar la moneda." });
        }
    };
    ...
    const handleDelete = async (satelliteId: string) => {
        try {
            ...
            toast({ variant: "destructive", title: "Error", description: "No se pudo eliminar la moneda." });
        }
    };
    ...
    return (
        <div className="space-y-6">
            <Card>
                <CardHeader>
                    <CardTitle>Añadir Nueva Moneda</CardTitle>
                </CardHeader>
                <CardContent>
                    <Form {...addForm}>
                        <form onSubmit={addForm.handleSubmit(onAddSubmit)} className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-start">
                                <FormField control={addForm.control} name="name" render={({ field }) => (
                                    <FormItem><FormLabel>Nombre de la Moneda</FormLabel><FormControl><Input placeholder="Bitcoin" {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                             ...
                             <FormField
                                ...
                                render={({ field }) => (
                                    ...
                                    <div className="space-y-0.5">
                                        ...
                                        <CardDescription>Activa un contador para esta moneda.</CardDescription>
                                    </div>
                                    <FormControl>
                                        <Switch
                                        checked={field.value}
                            ...
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <Label htmlFor="satellite-image-upload">Imagen de la Moneda</Label>
                                    <Input id="satellite-image-upload" type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'add')} />
                                    <p className="text-xs text-muted-foreground">Sube una imagen para la moneda. Si no se selecciona una, se usará una por defecto.</p>
                                </div>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>Monedas Existentes</CardTitle>
                    <CardDescription>Lista de todas las monedas disponibles en la plataforma.</CardDescription>
                </CardHeader>
                <CardContent>
            ...
            <Dialog open={!!editingSatellite} onOpenChange={(isOpen) => !isOpen && setEditingSatellite(null)}>
                <DialogContent className="sm:max-w-3xl">
                     <DialogHeader>
                        ...
                        <DialogDescription>
                           Realiza cambios en la moneda. Haz clic en guardar cuando termines.
                        </DialogDescription>
                    </DialogHeader>
                    <Form {...editForm}>
                        <form onSubmit={editForm.handleSubmit(onEditSubmit)} className="space-y-4 py-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <FormField control={editForm.control} name="name" render={({ field }) => (
                                    <FormItem><FormLabel>Nombre de la Moneda</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                            ...
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <Label htmlFor="edit-satellite-image-upload">Imagen de la Moneda</Label>
